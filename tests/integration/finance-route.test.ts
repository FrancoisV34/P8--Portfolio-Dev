...
describe('route finance privée', () => {
  ...
  it('versionne puis lance une simulation privée sans modifier le journal réel', async () => {
    ...
    expect(assumption.snapshot).toMatchObject({ months: 12, monthlyHouseholdIncomeCents: 20_000, businesses: firstActivity ? [expect.objectContaining({ id: firstActivity.id, charges: [expect.objectContaining({ name: 'Charge route synthétique', frequency: 'quarterly' })] })] : [] });
