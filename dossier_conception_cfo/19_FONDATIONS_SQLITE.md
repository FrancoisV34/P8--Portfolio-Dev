...
- Tables Better Auth `user`, `session`, `account` et `verification`, puis tables financières `finance_economic_entities`, `finance_accounts`, `finance_categories`, `finance_transactions`, `finance_monthly_budgets`, `finance_safety_reserves`, `finance_safety_reserve_accounts`, `finance_recurring_commitments`, `finance_gomining_scenarios`, `finance_gomining_contribution_phases`, `finance_gomining_scenario_versions`, `finance_wealth_assets`, `finance_wealth_asset_valuations`, `finance_wealth_debts`, `finance_wealth_debt_balances`, `finance_business_activities`, `finance_business_monthly_metrics`, `finance_business_entity_monthly_cash`, `finance_business_monthly_provisions`, `finance_goals`, `finance_projects`, `finance_project_capacity` et `finance_regulatory_rules`. Elles couvrent les soldes d’ouverture datés, les catégories revenu/dépense, les mouvements signés, les budgets mensuels, une réserve issue de comptes sélectionnés, les engagements mensuels, les hypothèses GoMining versionnées, le premier bilan patrimoine daté, les observations business mensuelles isolées du foyer, dont MRR, clients actifs et temps de maintenance facultatifs, les provisions déclaratives, les intentions manuelles d'objectifs/projets et le registre réglementaire manuel.
...
- `npm run db:backup -- nouveau-fichier.sqlite` produit une sauvegarde SQLite cohérente et vérifiée via le mécanisme natif, sans recopier WAL/SHM. `npm run db:restore -- sauvegarde.sqlite nouveau-fichier.sqlite` vérifie sa source et n’accepte qu’une destination absente, distincte de la base active et hors fichiers publics. Les fichiers créés ont des permissions privées et restent ignorés par Git.
...
## État de la fondation et limites restantes

L05 reste une fondation évolutive : les conventions TH et les autres modèles
seront complétés avec les prochains lots. L01/L03, L07 et L08 sont réalisés ;
les écrans privés de budget sont utilisables. La sauvegarde/restauration locale
de L06 est vérifiée. Depuis le 16 septembre 2026, l'environnement Fly.io est
en service avec HTTPS, stockage persistant et contrôle de santé ; une
sauvegarde téléchargée a été contrôlée puis restaurée dans une copie isolée.
Le confinement de conservation hors appareil et l'automatisation des copies
restent à mettre en place avant de considérer la reprise après incident comme
consolidée.

L07 et L08 sont réalisés localement : les formulaires privés de comptes, catégories et transactions sont ouverts, les transferts restent atomiques et le journal explique les totaux. Le dashboard mensuel compare prévu/réel sans tendance inventée ; il calcule la réserve depuis les comptes sélectionnés et distingue les engagements prévus des paiements explicitement rattachés. Les migrations `0003_safety-reserve-and-commitments` à `0016_regulatory-rule-versions` conservent les données existantes et ajoutent les hypothèses GoMining, leur lien facultatif à une catégorie de budget, leurs révisions immuables, puis les actifs, valorisations et états de dette datés et enfin les observations business. La migration `0009` convertit les actifs existants en positions manuelles et ajoute une position BTC GoMining observée, unique par propriétaire, qui exige une classe crypto et une quantité entière de satoshis. La migration `0010` ajoute uniquement les activités business, leurs métriques mensuelles et les états de cash par entité ; elle ne crée aucune donnée. La migration `0011` ajoute sans réécriture les champs facultatifs MRR, clients actifs et temps de maintenance ; l'ARR est calculé dans le domaine, jamais stocké ni déduit du CA. La migration `0012` ajoute les objectifs et projets manuels ; un déclencheur refuse le lien d'un projet à un objectif d'un autre propriétaire. La migration `0013` ajoute une capacité mensuelle unique par propriétaire pour le suivi informatif des projets actifs. Les migrations `0014` et `0015` ajoutent respectivement les provisions business déclaratives et le registre réglementaire manuel, sans calcul fiscal ni donnée préremplie. La migration `0016` transforme chaque règle existante en première révision et interdit ensuite toute modification ou suppression SQL d’une révision. Des contraintes et déclencheurs SQLite empêchent les liens d’un autre propriétaire, de catégorie incompatible, hors période, la réécriture d’une révision, le rattachement d'un actif, d'une valorisation ou d'un état de dette à une ressource d'un autre propriétaire, ainsi que le rattachement d'une activité ou d'un état business à une entité personnelle ou étrangère. Les routes `/finance/*` et `/api/finance/*` exigent le contrôle du propriétaire ; sans configuration locale, elles conservent un refus temporaire en `503`.
